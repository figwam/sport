...
class OpenIDInfoDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  protected def openIDInfoQuery(loginInfo: LoginInfo) = for {
    ...
    dbOpenIDInfo <- slickOpenIDInfos if dbOpenIDInfo.idLoginInfo === dbLoginInfo.id
  } yield dbOpenIDInfo
  ...
  def save(loginInfo: LoginInfo, authInfo: OpenIDInfo): Future[OpenIDInfo] = {
    val query = loginInfoQuery(loginInfo).joinLeft(slickOpenIDInfos).on(_.id === _.idLoginInfo)
  ...
  def remove(loginInfo: LoginInfo): Future[Unit] = {
    ...
    val openIDInfoSubQuery = slickOpenIDInfos.filter(_.idLoginInfo in loginInfoQuery(loginInfo).map(_.id))
